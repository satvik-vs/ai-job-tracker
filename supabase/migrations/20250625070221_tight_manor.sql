/*
  ...
  1. Changes
    - Ensure request_id column exists and is indexed
    - Allow job_application_id to be nullable for N8N responses
...
-- Create more flexible policies for AI generations
CREATE POLICY "Users can read AI generations"
  ...
  USING (
    ...
    OR
    -- Allow if job_application_id starts with user's ID or request pattern
    job_application_id LIKE 'n8n-%'
    OR
    job_application_id LIKE 'error-%'
  );
...
CREATE POLICY "Users can insert AI generations"
  ...
  WITH CHECK (
    ...
    job_application_id IS NULL
    OR
    -- Allow if job_application_id starts with request pattern
    job_application_id LIKE 'n8n-%'
    OR
    job_application_id LIKE 'error-%'
  );
...
INSERT INTO ai_generations (
  ...
  'n8n-test-' || extract(epoch from now()),
  false
